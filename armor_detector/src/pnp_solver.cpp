...
namespace
{
// CameraInfo/d is variable length: 5 for plumb_bob, 8 for rational_polynomial,
// and empty for an uncalibrated camera. Sizing the Mat from the vector avoids
// reading past the end of a short one.
cv::Mat toDistCoeffs(const std::vector<double> & d)
{
  if (d.empty()) {
    return cv::Mat::zeros(1, 5, CV_64F);
  }
  return cv::Mat(1, static_cast<int>(d.size()), CV_64F, const_cast<double *>(d.data())).clone();
}
}  // namespace
...
: camera_matrix_(cv::Mat(3, 3, CV_64F, const_cast<double *>(camera_matrix.data())).clone()),
  dist_coeffs_(toDistCoeffs(dist_coeffs))
{
  // Unit: m
