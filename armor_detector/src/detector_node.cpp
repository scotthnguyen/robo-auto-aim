...
std::unique_ptr<Detector> ArmorDetectorNode::initDetector()
{
  ...
  // Init YOLO detector if armor.onnx is present — replaces classical CV pipeline
  auto yolo_model_path = pkg_path + "/model/armor.onnx";
  std::ifstream yolo_check(yolo_model_path);
  if (yolo_check.good()) {
    yolo_check.close();
    float yolo_conf = static_cast<float>(this->declare_parameter("yolo_confidence", 0.5));
    float yolo_nms = static_cast<float>(this->declare_parameter("yolo_nms_threshold", 0.45));
    detector->yolo_detector = std::make_unique<YoloDetector>(yolo_model_path, yolo_conf, yolo_nms);
    RCLCPP_INFO(this->get_logger(), "YOLO armor detector loaded.");
  } else {
    RCLCPP_INFO(this->get_logger(), "No YOLO model found — using classical CV detector.");
  }
...
RCLCPP_COMPONENTS_REGISTER_NODE(sn_auto_aim::ArmorDetectorNode)
